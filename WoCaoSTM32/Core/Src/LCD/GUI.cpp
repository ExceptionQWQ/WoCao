...
{  
	...
	{		
		...
		{
			if(size==12)temp=asc2_1206_LCD[num][pos];//����1206����
			else temp=asc2_1608_LCD[num][pos];		 //����1608����
			for(t=0;t<size/2;t++)
		    {                 
...
void LCD_ShowSignedNum(u16 x, u16 y, u8 size, int num, u8 len)
{
	if (num < 0)
	{
		LCD_ShowChar(x, y, BLACK, WHITE, '-', size, 0);
	}
	else
	{
		LCD_ShowChar(x, y, BLACK, WHITE, '+', size, 0);
	}
	LCD_ShowNum(x + size/2, y, abs(num), len - 1, size, POINT_COLOR);
}
...
{         	
	...
	for(t=0;t<len;t++)
	{
		temp=(num/mypow_LCD(10,len-t-1))%10;
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,color,BACK_COLOR,' ',size,0);
				continue;
			}else enshow=1; 
...
void Gui_Drawbmp16(u16 x,u16 y,const unsigned char *p) //��ʾ40*40 QQͼƬ
{
	...
	LCD_SetWindows(x,y,x+35-1,y+70-1);//��������
    for(i=0;i<35*70;i++)
	{	
	 	picL=*(p+i*2);	//���ݵ�λ��ǰ
		picH=*(p+i*2+1);				
